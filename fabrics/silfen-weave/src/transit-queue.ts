...
function enqueueImpl(state: QueueState, request: TransitRequest): EnqueueResult {
...
function insertSorted(entries: TransitQueueEntry[], entry: TransitQueueEntry): void {
...
function cancelByEntityImpl(state: QueueState, entityId: string): number {
  let removed = 0;
