 ...
 * (LatticeNodeRegistry, FrequencyLockService) and emits transit events.
 */
...
/** Result of a lock state transition within the corridor's lock port. */
export interface CorridorLockTransition {
  readonly to: string;
}
...
export interface CorridorLockPort {
  ...
  updateCoherence(lockId: string, coherence: number): CorridorLockTransition | null;
  completeLock(lockId: string): void;
  abortLock(lockId: string, reason: string): void;
  triggerPartialCollapse(lockId: string, reason: string): void;
}
...
const TERMINAL_PHASES: ReadonlyArray<CorridorPhase> = ['arrived', 'aborted', 'collapsed'];

// ─── Factory ───────────────────────────────────────────────────────

export function createLatticeCorridorEngine(deps: LatticeCorridorDeps): LatticeCorridorEngine {
...
function abortCorridorImpl(
  ...
  if (corridor.phase !== 'route_validated' && corridor.phase !== 'lock_initiated') {
    throw corridorInvalidPhase(
      corridor.corridorId,
      corridor.phase,
      'route_validated|lock_initiated',
    );
  }
...
function tryGetCorridorImpl(state: CorridorState, corridorId: string): TransitCorridor | undefined {
...
function findActiveByEntity(state: CorridorState, entityId: string): MutableCorridor | undefined {
  for (const c of state.corridors.values()) {
