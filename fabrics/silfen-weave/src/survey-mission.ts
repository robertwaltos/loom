...
function evaluateTransitImpl(state: EngineState, missionId: string): MissionPhaseTransition | null {
...
function listByDynastyImpl(state: EngineState, dynastyId: string): ReadonlyArray<SurveyMission> {
...
function listByPhaseImpl(state: EngineState, phase: MissionPhase): ReadonlyArray<SurveyMission> {
  const result: SurveyMission[] = [];
  for (const m of state.missions.values()) {
