...
function getStatsImpl(state: PortalState): WorldPortalStats {
  ...
  return { totalPortals: state.portals.size, openPortals: open, closedPortals: closed, unstablePortals: unstable };
}
...
function createImpl(state: PortalState, p: CreatePortalParams): PortalSnapshot {
  const portal: Portal = {
    portalId: state.deps.idGenerator.next(), worldA: p.worldA, worldB: p.worldB,
    stability: p.stability, status: 'closed', usageCount: 0,
...
function createWorldPortalRegistry(deps: WorldPortalDeps): WorldPortalRegistry {
  ...
  return {
    ...
    getPortal: (id) => { const p = state.portals.get(id); return p ? toSnapshot(p) : undefined; },
