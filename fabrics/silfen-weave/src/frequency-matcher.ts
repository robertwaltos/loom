...
function compareSignatures(
  a: MatchableSignature,
  b: MatchableSignature,
): SignatureMatch {
...
function computeCompatibility(
  ...
  const raw = WEIGHT_HARMONIC * harmonicOverlap
...
function detectDriftImpl(
  ...
  const primaryDrift = computePrimaryDriftNormalized(
    previous.primary,
    current.primary,
  );
  const harmonicDrift = computeHarmonicDrift(
    previous.harmonics,
    current.harmonics,
  );
  const fieldStrengthDrift = Math.abs(
    previous.fieldStrength - current.fieldStrength,
  );
...
function mergeThresholds(
  overrides?: Partial<DriftThreshold>,
): DriftThreshold {
  if (overrides === undefined) return DEFAULT_DRIFT_THRESHOLD;
  return {
    minorPrimaryDrift: overrides.minorPrimaryDrift
      ?? DEFAULT_DRIFT_THRESHOLD.minorPrimaryDrift,
    significantPrimaryDrift: overrides.significantPrimaryDrift
      ?? DEFAULT_DRIFT_THRESHOLD.significantPrimaryDrift,
    criticalPrimaryDrift: overrides.criticalPrimaryDrift
      ?? DEFAULT_DRIFT_THRESHOLD.criticalPrimaryDrift,
    harmonicDriftFloor: overrides.harmonicDriftFloor
      ?? DEFAULT_DRIFT_THRESHOLD.harmonicDriftFloor,
  };
}
...
function computePrimaryDriftNormalized(
  previous: bigint,
  current: bigint,
): number {
