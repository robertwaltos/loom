...
function marsNode(): RegisterNodeParams {
  return {
    nodeId: 'node-mars',
    worldId: 'world-mars',
    signature: { primary: 432n, harmonics: [864, 1320, 1728], fieldStrength: 0.8 },
...
describe('LatticeNode beacon status', () => {
  ...
  it('allows forward transitions only', () => {
    ...
    expect(() => registry.setBeaconStatus('node-earth', 'active')).toThrow('cannot transition');
  });

  it('rejects status change with no beacon', () => {
    const { registry } = createTestRegistry();
    registry.registerNode(earthNode());
    expect(() => registry.setBeaconStatus('node-earth', 'degraded')).toThrow('no beacon deployed');
  });
});
