...
/**
 * Minimal CorridorLockPort stub for corridor tests.
 *
 * Tracks coherence per lock and transitions to 'transit_executing'
 * when coherence >= 0.95 (matching the lock threshold from the
 * frequency-lock module).
 */
function createTestLockPort(): CorridorLockPort {
  const locks = new Map<string, { coherence: number; status: string }>();
  const TRANSIT_THRESHOLD = 0.98;

  return {
    initiateLock(params): void {
      locks.set(params.lockId, { coherence: 0, status: 'attuning' });
    },
    updateCoherence(lockId, coherence): CorridorLockTransition | null {
      const lock = locks.get(lockId);
      if (lock === undefined) return null;
      const previousStatus = lock.status;
      lock.coherence = coherence;
      if (coherence > TRANSIT_THRESHOLD && previousStatus !== 'transit_executing') {
        lock.status = 'transit_executing';
        return { to: 'transit_executing' };
      }
      return null;
    },
    completeLock(lockId): void {
      const lock = locks.get(lockId);
      if (lock !== undefined) lock.status = 'completed';
    },
    abortLock(lockId): void {
      const lock = locks.get(lockId);
      if (lock !== undefined) lock.status = 'aborted';
    },
    triggerPartialCollapse(lockId): void {
      const lock = locks.get(lockId);
      if (lock !== undefined) lock.status = 'collapsed';
    },
  };
}
