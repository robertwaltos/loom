...
export function createTransitAnomalyDetector(
  ...
  return {
    recordCoherence: (l, e, c) => { recordCoherenceImpl(state, l, e, c); },
    startTracking: (l, e) => { startTrackingImpl(state, l, e); },
    stopTracking: (l) => { stopTrackingImpl(state, l); },
...
function checkCoherenceJump(
  ...
  const severity: AnomalySeverity = delta > state.config.maxCoherenceStep * 2
    ? 'critical'
    : 'warning';
  ...
  emitAnomaly(state, telemetry, {
    type: 'coherence_jump',
    severity,
    description: 'Coherence changed by ' + delta.toFixed(4)
      + ' (max ' + state.config.maxCoherenceStep.toFixed(4) + ')',
...
function checkVelocityViolation(
  ...
  emitAnomaly(state, telemetry, {
    ...
    severity: 'warning',
    description: 'Coherence velocity ' + velocity.toExponential(4)
      + '/us exceeds max ' + state.config.maxVelocityPerUs.toExponential(4) + '/us',
    coherenceValue: coherence,
    previousCoherence: previous.coherence,
    detectedAt: now,
  });
...
function checkOscillation(
  ...
  emitAnomaly(state, telemetry, {
    type: 'oscillation',
    severity: 'critical',
    description: 'Detected ' + String(reversals) + ' direction reversals in '
      + String(windowSize) + ' readings',
...
function countDirectionReversals(
  readings: ReadonlyArray<CoherenceReading>,
): number {
  let reversals = 0;
  let lastDirection = 0;
...
function getAnomaliesImpl(
  state: DetectorState,
  lockId: string,
): ReadonlyArray<TransitAnomaly> {
  const telemetry = state.locks.get(lockId);
  if (telemetry === undefined) return [];
  return [...telemetry.anomalies];
}

function getAllAnomaliesImpl(
  state: DetectorState,
): ReadonlyArray<TransitAnomaly> {
