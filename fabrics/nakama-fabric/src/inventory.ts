...
export function createInventoryService(deps: InventoryDeps): InventoryService {
...
function getOrCreateInventory(state: ServiceState, dynastyId: string): Map<string, MutableItem> {
...
function inventoryImpl(state: ServiceState, dynastyId: string): ReadonlyArray<InventoryItem> {
  const inv = state.inventories.get(dynastyId);
