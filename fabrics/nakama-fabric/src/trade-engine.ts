...
const TERMINAL_PHASES: ReadonlyArray<TradePhase> = [
...
function mergeConfig(
  overrides?: Partial<TradeEngineConfig>,
): TradeEngineConfig {
  if (overrides === undefined) return DEFAULT_TRADE_CONFIG;
  return {
    defaultExpirationUs: overrides.defaultExpirationUs
      ?? DEFAULT_TRADE_CONFIG.defaultExpirationUs,
  };
}
...
function cancelImpl(
  ...
  const isParty = cancelledBy === trade.initiatorId
    || cancelledBy === trade.counterpartyId;
  if (!isParty) {
...
function listByAccountImpl(
  ...
  for (const trade of state.trades.values()) {
    checkExpiry(state, trade);
    const involved = trade.initiatorId === accountId
      || trade.counterpartyId === accountId;
    if (involved) {
