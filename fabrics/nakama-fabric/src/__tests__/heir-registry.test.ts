...
function createTestRegistry(dynastyOverrides?: Partial<Record<string, Partial<HeirDynastyInfo>>>): {
...
describe('Heir registry revocation', () => {
  ...
  it('handles revocation of non-existent heir gracefully', () => {
    const { registry } = createTestRegistry();
    expect(() => {
      registry.revokeHeir('parent-1', 'nonexistent');
    }).not.toThrow();
  });
...
describe('Heir registry claim validation', () => {
  it('rejects claim for non-completed dynasty', () => {
    ...
    expect(() => registry.claimInheritance('parent-1', 'heir-1')).toThrow('not completed');
  });
  ...
  it('rejects claim from non-registered heir', () => {
    ...
    expect(() => registry.claimInheritance('parent-1', 'heir-1')).toThrow('not registered');
  });
  ...
  it('rejects double inheritance', () => {
    ...
    registry.claimInheritance('parent-1', 'heir-1');
    expect(() => registry.claimInheritance('parent-2', 'heir-1')).toThrow('active inheritance');
  });
});
