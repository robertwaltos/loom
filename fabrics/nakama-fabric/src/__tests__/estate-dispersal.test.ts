...
function createAuctionWithLots(engine: EstateAuctionEngine) {
  ...
  engine.addLot('auction-1', {
    lotId: 'lot-castle', category: 'territory',
    description: 'Castle Caladan', minimumBid: 1000n,
  });
  engine.addLot('auction-1', {
    lotId: 'lot-sword', category: 'asset',
    description: 'Ducal Sword', minimumBid: 500n,
  });
  engine.addLot('auction-1', {
    lotId: 'lot-memorial', category: 'memorial',
    description: 'Memorial inscription', minimumBid: 100n,
  });
}
...
describe('EstateAuction lot management', () => {
  it('adds lots to auction', () => {
    ...
    const lot = engine.addLot('auction-1', {
      lotId: 'lot-1', category: 'asset',
      description: 'Ducal Ring', minimumBid: 500n,
    });
  ...
  it('rejects lots on completed auction', () => {
    ...
    expect(() => engine.addLot('auction-1', {
      lotId: 'late-lot', category: 'asset',
      description: 'Too late', minimumBid: 100n,
    })).toThrow();
  });
});
...
describe('EstateAuction bidding', () => {
  it('accepts valid bid above minimum', () => {
    ...
    const bid = engine.placeBid('auction-1', {
      bidId: 'bid-1', lotId: 'lot-castle',
      bidderId: 'paul-atreides', amount: 1500n,
    });
  ...
  it('rejects bid at or below minimum', () => {
    const { engine } = createTestAuctionEngine();
    createAuctionWithLots(engine);
    expect(() => engine.placeBid('auction-1', {
      bidId: 'bid-1', lotId: 'lot-castle',
      bidderId: 'paul-atreides', amount: 1000n,
    })).toThrow();
  });
  ...
  it('rejects bid at or below current highest', () => {
    ...
    engine.placeBid('auction-1', {
      bidId: 'bid-1', lotId: 'lot-castle',
      bidderId: 'paul-atreides', amount: 2000n,
    });
    expect(() => engine.placeBid('auction-1', {
      bidId: 'bid-2', lotId: 'lot-castle',
      bidderId: 'feyd-harkonnen', amount: 2000n,
    })).toThrow();
  });
  ...
  it('accepts bid above current highest', () => {
    ...
    engine.placeBid('auction-1', {
      bidId: 'bid-2', lotId: 'lot-castle',
      bidderId: 'feyd-harkonnen', amount: 3000n,
    });
...
describe('EstateAuction memorial bids', () => {
  it('stores memorial text on memorial lots', () => {
    ...
    createAuctionWithLots(engine);
    engine.placeBid('auction-1', {
      bidId: 'bid-1', lotId: 'lot-memorial',
      bidderId: 'paul-atreides', amount: 200n,
      memorialText: 'The sleeper has awakened.',
    });
    const memorial = engine.getActiveLots('auction-1')
      .find((l) => l.lotId === 'lot-memorial');
  ...
  it('ignores memorial text on non-memorial lots', () => {
    ...
    createAuctionWithLots(engine);
    engine.placeBid('auction-1', {
      bidId: 'bid-1', lotId: 'lot-sword',
      bidderId: 'paul-atreides', amount: 600n,
      memorialText: 'Should be ignored',
    });
    const sword = engine.getActiveLots('auction-1')
      .find((l) => l.lotId === 'lot-sword');
...
describe('EstateAuction completion', () => {
  ...
  it('marks lots without bids as liquidated', () => {
    ...
    createAuctionWithLots(engine);
    advanceThroughAllPhases(engine, clock, 'auction-1');
    const liquidated = engine.getAuction('auction-1').lots
      .filter((l) => l.status === 'liquidated');
