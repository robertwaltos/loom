...
describe('MARKS supply caps', () => {
  ...
  it('rejects FIRST_CONTACT beyond cap', () => {
    ...
    expect(() => reg.award({
      markType: 'FIRST_CONTACT', dynastyId: 'd4', chronicleEntryRef: 'h4',
  ...
  it('error is MarkError with correct code', () => {
    ...
    for (let i = 0; i < 3; i++) {
      reg.award({
        markType: 'FIRST_CONTACT', dynastyId: 'd' + String(i), chronicleEntryRef: 'h' + String(i),
      });
    }
    try {
...
describe('MARKS WORLD uniqueness', () => {
  it('allows one WORLD mark per world', () => {
    const reg = createTestRegistry();
    const mark = reg.award({
      markType: 'WORLD', dynastyId: 'dyn-1', chronicleEntryRef: 'h1', worldId: 'earth',
    });
  ...
  it('rejects duplicate WORLD mark for same world', () => {
    ...
    expect(() => reg.award({
      markType: 'WORLD', dynastyId: 'd2', chronicleEntryRef: 'h2', worldId: 'earth',
...
describe('MARKS chronicle ref uniqueness', () => {
  it('rejects duplicate chronicle reference', () => {
    ...
    expect(() => reg.award({
      markType: 'FOUNDING', dynastyId: 'd2', chronicleEntryRef: 'same-hash',
...
describe('MARKS all types awardable', () => {
  const markTypes: ReadonlyArray<MarkType> = [
    'FOUNDING', 'SURVEY', 'WORLD', 'DEFENCE', 'SURVIVOR', 'FIRST_CONTACT',
  ];
