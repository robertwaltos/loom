...
describe('TradeEngine — proposal', () => {
  ...
  it('rejects self-trade', () => {
    ...
    expect(() => engine.propose(makeProposal({ counterpartyId: 'alice' }))).toThrow(
  ...
  it('rejects zero offer amount', () => {
    const engine = createTradeEngine(makeDeps());
    expect(() => engine.propose(makeProposal({ offerAmount: 0n }))).toThrow(
  ...
  it('rejects when insufficient escrow balance', () => {
    ...
    const engine = createTradeEngine(deps);
    expect(() => engine.propose(makeProposal({ offerAmount: 100n }))).toThrow(
...
describe('TradeEngine — cancellation', () => {
  ...
  it('rejects cancel from non-party', () => {
    ...
    expect(() => engine.cancel('trade-1', 'charlie')).toThrow('Only trade parties can cancel');
  });
  ...
  it('rejects cancel on terminal trade', () => {
    ...
    expect(() => engine.cancel('trade-1', 'alice')).toThrow('already terminal');
  });
});
