...
describe('ContinuityBondEngine initialization', () => {
  ...
  it('rejects duplicate records', () => {
    ...
    expect(() => engine.initializeRecord('house-atreides')).toThrow('already exists');
  });
...
describe('ContinuityBondEngine spending rejections', () => {
  it('rejects spend when bond count is zero', () => {
    ...
    engine.spendBond('d1', 'dormant_30');
    engine.spendBond('d1', 'dormant_30');
    expect(() => engine.spendBond('d1', 'dormant_30')).toThrow('Invalid continuity transition');
  });

  it('rejects spend from active state', () => {
    const { engine } = createTestEngine();
    engine.initializeRecord('d1');
    expect(() => engine.spendBond('d1', 'active')).toThrow('Invalid continuity transition');
  });

  it('rejects spend from redistribution state', () => {
    const { engine } = createTestEngine();
    engine.initializeRecord('d1');
    expect(() => engine.spendBond('d1', 'redistribution')).toThrow('Invalid continuity transition');
  });

  it('rejects spend from completed state', () => {
    const { engine } = createTestEngine();
    engine.initializeRecord('d1');
    expect(() => engine.spendBond('d1', 'completed')).toThrow('Invalid continuity transition');
  });

  it('rejects spend from vigil state', () => {
    const { engine } = createTestEngine();
    engine.initializeRecord('d1');
    expect(() => engine.spendBond('d1', 'vigil')).toThrow('Invalid continuity transition');
  });

  it('rejects spend from heir_activated state', () => {
    const { engine } = createTestEngine();
    engine.initializeRecord('d1');
    expect(() => engine.spendBond('d1', 'heir_activated')).toThrow('Invalid continuity transition');
  });

  it('rejects spend from legacy_npc state', () => {
    const { engine } = createTestEngine();
    engine.initializeRecord('d1');
    expect(() => engine.spendBond('d1', 'legacy_npc')).toThrow('Invalid continuity transition');
  });

  it('throws for unknown dynasty on spendBond', () => {
    const { engine } = createTestEngine();
    expect(() => engine.spendBond('nope', 'dormant_30')).toThrow('not found');
  });
});
