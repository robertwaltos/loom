...
describe('Subscription tier ordering invariants', () => {
  it('price increases with tier', () => {
    ...
    expect(TIER_CONFIGS.patron.priceUsdMonthly).toBeGreaterThan(TIER_CONFIGS.accord.priceUsdMonthly);
    expect(TIER_CONFIGS.herald.priceUsdMonthly).toBeGreaterThan(TIER_CONFIGS.patron.priceUsdMonthly);
  });

  it('dynasty slots increase with tier', () => {
    expect(TIER_CONFIGS.accord.maxActiveDynasties).toBeGreaterThan(TIER_CONFIGS.free.maxActiveDynasties);
    expect(TIER_CONFIGS.patron.maxActiveDynasties).toBeGreaterThan(TIER_CONFIGS.accord.maxActiveDynasties);
    expect(TIER_CONFIGS.herald.maxActiveDynasties).toBeGreaterThan(TIER_CONFIGS.patron.maxActiveDynasties);
  });

  it('stipend increases with tier', () => {
    expect(TIER_CONFIGS.accord.kalonMonthlyStipendMicro > TIER_CONFIGS.free.kalonMonthlyStipendMicro).toBe(true);
    expect(TIER_CONFIGS.patron.kalonMonthlyStipendMicro > TIER_CONFIGS.accord.kalonMonthlyStipendMicro).toBe(true);
    expect(TIER_CONFIGS.herald.kalonMonthlyStipendMicro > TIER_CONFIGS.patron.kalonMonthlyStipendMicro).toBe(true);
  });

  it('grace days increase with tier', () => {
    expect(TIER_CONFIGS.accord.continuityGraceDays).toBeGreaterThan(TIER_CONFIGS.free.continuityGraceDays);
    expect(TIER_CONFIGS.patron.continuityGraceDays).toBeGreaterThan(TIER_CONFIGS.accord.continuityGraceDays);
    expect(TIER_CONFIGS.herald.continuityGraceDays).toBeGreaterThan(TIER_CONFIGS.patron.continuityGraceDays);
  });
});
