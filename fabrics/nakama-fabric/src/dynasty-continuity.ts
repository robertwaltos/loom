...
export function createContinuityEngine(deps: {
  ...
  return {
    ...
    registerHeir: (id, heirId) => {
      registerHeirImpl(state, id, heirId);
    },
    removeHeir: (id, heirId) => {
      removeHeirImpl(state, id, heirId);
    },
    updateSubscriptionTier: (id, tier) => {
      updateTierImpl(state, id, tier);
    },
...
function tryGetRecordImpl(state: EngineState, dynastyId: string): ContinuityRecord | undefined {
  const record = state.records.get(dynastyId);
...
function recordLoginImpl(state: EngineState, dynastyId: string): ContinuityTransition | null {
...
function registerHeirImpl(state: EngineState, dynastyId: string, heirDynastyId: string): void {
  const record = getMutableRecord(state, dynastyId);
...
function removeHeirImpl(state: EngineState, dynastyId: string, heirDynastyId: string): void {
  const record = getMutableRecord(state, dynastyId);
  record.heirDynastyIds = record.heirDynastyIds.filter((id) => id !== heirDynastyId);
}

function updateTierImpl(state: EngineState, dynastyId: string, tier: SubscriptionTier): void {
...
function evaluateAllImpl(state: EngineState): ReadonlyArray<ContinuityTransition> {
...
function evaluateSingleRecord(record: MutableRecord, now: number): ContinuityTransition | null {
...
function evaluateStateTransition(
  ...
  switch (record.state) {
    ...
    case 'dormant_30':
      return transitionIf(
        record,
        inactiveDays >= DORMANCY_60_DAYS,
        'dormant_60',
        now,
        'Inactive for 60 days — Assembly notified',
      );
    ...
    case 'continuity_triggered':
      return transitionIf(
        record,
        inactiveDays >= HARD_DEADLINE_DAYS,
        'redistribution',
        now,
        'Day 180 — redistribution begins',
      );
...
function evaluateDormant60(
  ...
  return transitionIf(
    record,
    inactiveDays >= MINIMUM_CONTINUITY_DAYS,
    'continuity_triggered',
    now,
    'Day 91 — continuity triggered (free tier)',
  );
}
...
function evaluateGraceWindow(
  ...
  return transitionIf(
    ...
    'continuity_triggered',
    now,
    `Day ${String(threshold)} — continuity triggered`,
  );
}

function evaluateHeirWindow(record: MutableRecord, now: number): ContinuityTransition | null {
  ...
  return applyTransition(record, 'legacy_npc', now, 'No heir claimed within 2 years — Legacy NPC');
}

// ─── Special Transitions ────────────────────────────────────────────

function declareVigilImpl(state: EngineState, dynastyId: string): ContinuityTransition {
  const record = getMutableRecord(state, dynastyId);
...
function activateHeirImpl(
  ...
  return applyTransition(record, 'heir_activated', now, `Heir ${heirDynastyId} activated`);
}

function convertToLegacyNpcImpl(state: EngineState, dynastyId: string): ContinuityTransition {
  const record = getMutableRecord(state, dynastyId);
  assertState(record, 'completed', 'legacy_npc');
  const now = state.clock.nowMicroseconds();
  return applyTransition(record, 'legacy_npc', now, 'Dynasty converted to Legacy NPC');
}

function completeRedistributionImpl(state: EngineState, dynastyId: string): ContinuityTransition {
  const record = getMutableRecord(state, dynastyId);
  assertState(record, 'redistribution', 'completed');
  const now = state.clock.nowMicroseconds();
  record.completedAt = now;
  return applyTransition(record, 'completed', now, 'Estate dispersal complete — dynasty completed');
}
...
function daysUntilNextTransitionImpl(state: EngineState, dynastyId: string): number | null {
  const record = getMutableRecord(state, dynastyId);
  const now = state.clock.nowMicroseconds();
  const inactiveDays = daysSince(record.lastLoginAt, now);
