...
export interface EstateAuctionEngine {
  ...
  canBid(auctionId: string, bidderId: string, heirIds: ReadonlyArray<string>,
    allyIds: ReadonlyArray<string>): boolean;
  count(): number;
}
...
const PHASE_ORDER: ReadonlyArray<AuctionPhase> = [
  'heirs', 'allies', 'assembly', 'liquidation',
];
...
export function createEstateAuctionEngine(deps: {
  ...
  return {
    ...
    canBid: (id, bidderId, heirIds, allyIds) =>
      canBidImpl(state, id, bidderId, heirIds, allyIds),
...
function getActiveLotsImpl(
  state: EngineState,
  auctionId: string,
): ReadonlyArray<AuctionLot> {
  const auction = getMutableAuction(state, auctionId);
  return auction.lots
...
function getMemorialWinnerImpl(
  state: EngineState,
  auctionId: string,
): AuctionLot | undefined {
  const auction = getMutableAuction(state, auctionId);
  const memorial = auction.lots.find(
    (l) => l.category === 'memorial' && l.status === 'sold',
  );
