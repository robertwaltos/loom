...
function initializeRecordImpl(state: BondEngineState, dynastyId: string): ContinuityBondRecord {
...
function getRecordImpl(state: BondEngineState, dynastyId: string): ContinuityBondRecord {
...
function getMutableRecord(state: BondEngineState, dynastyId: string): MutableBondRecord {
  const record = state.records.get(dynastyId);
  if (!record) throw continuityRecordNotFound(dynastyId);
  return record;
}
