...
function isDuplicate(state: RegistryState, parentId: string, heirId: string): boolean {
...
function removeParentIndex(state: RegistryState, heirId: string, parentId: string): void {
  const parents = state.parentIndex.get(heirId);
...
function validateClaim(state: RegistryState, parentDynastyId: string, heirDynastyId: string): void {
...
function isRegisteredHeir(state: RegistryState, parentId: string, heirId: string): boolean {
  const heirs = state.declarations.get(parentId);
  if (heirs === undefined) return false;
  return heirs.some((d) => d.heirDynastyId === heirId);
}
...
function getHeirsImpl(state: RegistryState, parentId: string): ReadonlyArray<HeirDeclaration> {
  return state.declarations.get(parentId) ?? [];
}

function getParentsImpl(state: RegistryState, heirId: string): ReadonlyArray<string> {
  const parents = state.parentIndex.get(heirId);
