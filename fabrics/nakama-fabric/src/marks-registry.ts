...
function awardMark(
  state: RegistryState,
  deps: MarksRegistryDeps,
  params: AwardMarkParams,
): Mark {
...
function validateWorldUniqueness(state: RegistryState, params: AwardMarkParams): void {
  ...
  if (state.worldMarks.has(worldId)) {
    throw new MarkError(
      'WORLD_MARK_EXISTS',
      `WORLD mark already exists for world ${worldId}`,
    );
  }
}
...
function validateChronicleRef(state: RegistryState, ref: string): void {
  if (state.chronicleRefs.has(ref)) {
    throw new MarkError(
      'DUPLICATE_CHRONICLE_REF',
      `Chronicle ref already used: ${ref}`,
    );
  }
}
...
function countByDynastyAndType(
  ...
  return (state.byDynasty.get(dynastyId) ?? []).filter(
    (m) => m.markType === markType,
  ).length;
}
