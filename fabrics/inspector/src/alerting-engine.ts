...
export function createAlertingEngine(deps: AlertingDeps): AlertingEngine {
...
function getStatusImpl(state: EngineState, name: string): AlertStatus | undefined {
...
function getAllStatusesImpl(state: EngineState): ReadonlyArray<AlertStatus> {
...
function getFiringImpl(state: EngineState): ReadonlyArray<AlertStatus> {
  const result: AlertStatus[] = [];
  for (const [name, rule] of state.rules.entries()) {
    const alert = state.alerts.get(name);
