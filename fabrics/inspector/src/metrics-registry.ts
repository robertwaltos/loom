...
function buildHistogramMetric(meta: MetricMeta, hs: HistogramState): HistogramMetric {
  return {
    ...
    getCount(lbl) {
      return hs.entries.get(labelKey(lbl))?.count ?? 0;
    },
    getSum(lbl) {
      return hs.entries.get(labelKey(lbl))?.sum ?? 0;
    },
    getMin(lbl) {
      return hs.entries.get(labelKey(lbl))?.min ?? 0;
    },
    getMax(lbl) {
      return hs.entries.get(labelKey(lbl))?.max ?? 0;
    },
    getMean(lbl) {
      return histogramMean(hs, lbl);
    },
  };
}
...
function histogramMean(hs: HistogramState, lbl?: Readonly<Record<string, string>>): number {
