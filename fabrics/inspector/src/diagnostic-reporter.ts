...
function addAlertRecommendations(
  recs: Recommendation[],
  alerts: AlertSummarySection,
): void {
  if (alerts.criticalCount > 0) {
    recs.push({
      priority: 'high',
      category: 'alerting',
      message: String(alerts.criticalCount) + ' critical alert(s) firing — immediate action required',
    });
  }
...
function getHistoryImpl(
  state: ReporterState,
  limit: number,
): ReadonlyArray<DiagnosticReport> {
