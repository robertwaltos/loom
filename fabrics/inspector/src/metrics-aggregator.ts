...
export function createMetricsAggregator(deps: AggregatorDeps): MetricsAggregator {
...
function addSample(track: MetricTrack, value: number, at: number, config: AggregatorConfig): void {
...
function getAggregationImpl(state: AggregatorState, name: string): AggregatedMetric | undefined {
  const track = state.tracks.get(name);
  if (track === undefined) return undefined;
  return buildAggregation(name, track, state.config);
}

function getAllAggregationsImpl(state: AggregatorState): ReadonlyArray<AggregatedMetric> {
...
function emptyAggregation(name: string, windowSizeUs: number): AggregatedMetric {
  return {
    name,
    windowSizeUs,
    sampleCount: 0,
...
function getWindowSamples(track: MetricTrack, config: AggregatorConfig): ReadonlyArray<Sample> {
...
function computeTrend(samples: ReadonlyArray<Sample>, threshold: number): TrendDirection {
...
function computeRate(samples: ReadonlyArray<Sample>): number | null {
