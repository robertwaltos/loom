...
): { engine: AlertingEngine; advanceTime: (us: number) => void; notifications: AlertNotification[]; setMetric: (name: string, val: number) => void } {
  let time = 1_000_000;
  const notifications: AlertNotification[] = [];
  const deps: AlertingDeps = {
    clock: { nowMicroseconds: () => time },
    getMetricValue: (name) => metrics[name],
    onAlert: (n) => { notifications.push(n); },
  };
  return {
    engine: createAlertingEngine(deps),
    advanceTime: (us: number) => { time += us; },
    notifications,
    setMetric: (name: string, val: number) => { metrics[name] = val; },
  };
}
