...
describe('NotificationChannel — subscribe', () => {
  it('subscribes a listener', () => {
    ...
    const result = channel.subscribe({ subscriberId: 'sub-1', topics: ['chronicle.entry'] }, () => {
      /* noop */
    });
  ...
  it('rejects duplicate subscriber', () => {
    const channel = createNotificationChannel(makeDeps());
    channel.subscribe({ subscriberId: 'sub-1', topics: ['a'] }, () => {
      /* noop */
    });
    expect(
      channel.subscribe({ subscriberId: 'sub-1', topics: ['b'] }, () => {
        /* noop */
      }),
    ).toBe(false);
  });
...
describe('NotificationChannel — publish', () => {
  it('publishes and delivers to matching subscriber', () => {
    ...
    channel.subscribe({ subscriberId: 'sub-1', topics: ['event.created'] }, (n) => {
      received.push(n);
    });
...
describe('NotificationChannel — subscriber queries', () => {
  it('gets subscriber by id', () => {
    const channel = createNotificationChannel(makeDeps());
    channel.subscribe({ subscriberId: 'sub-1', topics: ['a', 'b'] }, () => {
      /* noop */
    });
  ...
  it('lists all subscribers', () => {
    ...
    channel.subscribe({ subscriberId: 'sub-2', topics: ['b'] }, () => {
      /* noop */
    });
  ...
  it('lists subscribers by topic', () => {
    ...
    channel.subscribe({ subscriberId: 'sub-2', topics: ['b', 'c'] }, () => {
      /* noop */
    });
    channel.subscribe({ subscriberId: 'sub-3', topics: ['c'] }, () => {
      /* noop */
    });
...
describe('NotificationChannel — stats', () => {
  ...
  it('tracks aggregate stats', () => {
    ...
    channel.subscribe({ subscriberId: 'sub-2', topics: ['a'] }, () => {
      /* noop */
    });
    channel.publish({ topic: 'a', payload: '1' });
    channel.publish({ topic: 'b', payload: '2' });
