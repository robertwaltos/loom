...
function recoverImpl(state: ServiceState, request: RecoveryRequest): RecoveryResult {
...
function applyAndRecord(
  ...
  const applied = state.deps.applyPort.apply(snapshot.worldId, stateData, snapshot.tickNumber);
...
function verifyImpl(state: ServiceState, snapshotId: string): RecoveryCandidate | undefined {
...
function getHistoryImpl(state: ServiceState, limit: number): ReadonlyArray<RecoveryResult> {
  if (limit >= state.history.length) return [...state.history];
  return state.history.slice(state.history.length - limit);
}

function lastRecoveryImpl(state: ServiceState, worldId: string): RecoveryResult | undefined {
