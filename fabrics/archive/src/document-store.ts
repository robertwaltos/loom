...
interface DocumentStore {
  ...
  readonly getVersion: (collection: string, key: string, version: number) => Document | undefined;
...
function toDocument(versions: DocumentVersions, ver: MutableVersion): Document {
  return {
...
function putImpl(state: DocumentState, params: PutDocumentParams): Document {
...
function getImpl(state: DocumentState, collection: string, key: string): Document | undefined {
...
function listImpl(state: DocumentState, filter: DocumentFilter): readonly Document[] {
  const results: Document[] = [];
  for (const versions of state.documents.values()) {
    if (filter.collection !== undefined && versions.collection !== filter.collection) {
      continue;
    }
    if (filter.keyPrefix !== undefined && !versions.key.startsWith(filter.keyPrefix)) {
      continue;
    }
    const latest = versions.versions[versions.versions.length - 1];
    if (!latest) continue;
...
function removeImpl(state: DocumentState, collection: string, key: string): boolean {
...
function getVersionCountImpl(state: DocumentState, collection: string, key: string): number {
  const versions = state.documents.get(composeKey(collection, key));
