...
function sendDirectImpl(state: RouterState, params: RouteDirectParams): RoutedMessage {
  const delivered = state.deps.deliveryPort.deliver(params.connectionId, params.payload);
...
function broadcastImpl(state: RouterState, params: RouteBroadcastParams): RoutedMessage {
...
function sendGroupImpl(state: RouterState, params: RouteGroupParams): RoutedMessage {
...
function sendDynastyImpl(state: RouterState, params: RouteDynastyParams): RoutedMessage {
  const connections = state.deps.connectionPort.getConnectionsByDynasty(params.dynastyId);
  let count = 0;
...
function subscribeImpl(state: RouterState, connectionId: string, channel: string): boolean {
...
function addToConnectionChannels(state: RouterState, connectionId: string, channel: string): void {
...
function unsubscribeImpl(state: RouterState, connectionId: string, channel: string): boolean {
...
function getSubsImpl(state: RouterState, connectionId: string): ReadonlyArray<string> {
  const channels = state.connectionChannels.get(connectionId);
  return channels !== undefined ? [...channels] : [];
}

function getMembersImpl(state: RouterState, channel: string): ReadonlyArray<string> {
  const members = state.channelMembers.get(channel);
