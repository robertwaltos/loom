...
function registerImpl(state: TrackerState, connectionId: string): boolean {
...
function unregisterImpl(state: TrackerState, connectionId: string): boolean {
  return state.connections.delete(connectionId);
}

function maybeResetWindow(state: TrackerState, record: MutableRecord, now: number): void {
...
function recordTrafficImpl(state: TrackerState, params: RecordTrafficParams): boolean {
...
function isOverQuotaImpl(state: TrackerState, connectionId: string): boolean {
...
function resetWindowImpl(state: TrackerState, connectionId: string): boolean {
  const record = state.connections.get(connectionId);
  if (!record) return false;
  record.bytesSent = 0;
  record.bytesReceived = 0;
