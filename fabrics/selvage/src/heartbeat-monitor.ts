...
function classifyHealth(silenceUs: number, config: HeartbeatConfig): ConnectionHealth {
  if (silenceUs >= config.timeoutUs) return 'stale';
  if (silenceUs >= config.warningThresholdUs) return 'warning';
  return 'healthy';
}

function checkHealthImpl(state: HeartbeatState, connectionId: string): HealthCheck | undefined {
  const record = state.records.get(connectionId);
...
function getRecordImpl(state: HeartbeatState, connectionId: string): HeartbeatRecord | undefined {
  const record = state.records.get(connectionId);
  if (!record) return undefined;
  return {
    connectionId,
