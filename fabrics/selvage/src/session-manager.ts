...
function mergeConfig(overrides?: Partial<SessionManagerConfig>): SessionManagerConfig {
  if (overrides === undefined) return DEFAULT_SESSION_CONFIG;
  return {
    idleThresholdUs: overrides.idleThresholdUs ?? DEFAULT_SESSION_CONFIG.idleThresholdUs,
    maxSessionDurationUs:
      overrides.maxSessionDurationUs ?? DEFAULT_SESSION_CONFIG.maxSessionDurationUs,
    heartbeatIntervalUs:
      overrides.heartbeatIntervalUs ?? DEFAULT_SESSION_CONFIG.heartbeatIntervalUs,
  };
}

// ─── Create ─────────────────────────────────────────────────────────

function createImpl(state: ManagerState, params: CreateSessionParams): Session {
...
function terminateImpl(state: ManagerState, sessionId: string, _reason: string): boolean {
  const session = state.sessions.get(sessionId);
  if (session === undefined) return false;
  if (isTerminal(session.state)) return false;
...
function getSessionImpl(state: ManagerState, sessionId: string): Session | undefined {
  const session = state.sessions.get(sessionId);
...
function getByConnectionImpl(state: ManagerState, connectionId: string): Session | undefined {
  const sessionId = state.connectionIndex.get(connectionId);
  if (sessionId === undefined) return undefined;
  return getSessionImpl(state, sessionId);
}

function getByDynastyImpl(state: ManagerState, dynastyId: string): ReadonlyArray<Session> {
