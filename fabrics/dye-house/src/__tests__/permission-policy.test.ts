...
describe('PermissionPolicyEngine — resource matching', () => {
  it('matches exact resource', () => {
    ...
    engine.addPolicy({
      name: 'exact',
      effect: 'allow',
      resourcePattern: 'world/earth',
      action: 'read',
    });
    const result = engine.evaluate({
      subjectAttributes: {},
      resource: 'world/earth',
      action: 'read',
    });
    expect(result.decision).toBe('allowed');
  });
