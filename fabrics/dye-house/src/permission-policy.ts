...
export function createPermissionPolicyEngine(deps: PermissionPolicyDeps): PermissionPolicyEngine {
...
function addPolicyImpl(state: EngineState, params: CreatePolicyParams): Policy {
  const policy: Policy = {
...
function evaluateImpl(state: EngineState, request: EvaluationRequest): EvaluationResult {
...
function findMatchingPolicies(state: EngineState, request: EvaluationRequest): Policy[] {
  const result: Policy[] = [];
...
function evaluateCondition(cond: PolicyCondition, attrs: PolicyAttributes): boolean {
