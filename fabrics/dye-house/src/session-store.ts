...
function createImpl(state: StoreState, params: CreateSessionParams): SessionRecord {
...
function getImpl(state: StoreState, sessionId: string): SessionRecord | undefined {
...
function setDataImpl(state: StoreState, sessionId: string, key: string, value: string): boolean {
  const session = state.sessions.get(sessionId);
