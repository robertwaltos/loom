...
function evaluateTier(subject: PermissionSubject, rule: PermissionRule, reasons: string[]): void {
  const subjectRank = TIER_RANK[subject.tier];
  const requiredRank = TIER_RANK[rule.minimumTier];
  if (subjectRank < requiredRank) {
    reasons.push('Requires ' + rule.minimumTier + ' tier or higher, has ' + subject.tier);
  }
}

// ─── Status Evaluation ─────────────────────────────────────────────

function evaluateStatus(subject: PermissionSubject, rule: PermissionRule, reasons: string[]): void {
  ...
  if (!match) {
    ...
    reasons.push('Requires status ' + allowed + ', has ' + subject.status);
  }
}
...
function appliesToAction(predicate: CustomPredicate, action: string): boolean {
