...
function createPlayerConnectOrchestrator(
  deps: PlayerConnectDeps,
): PlayerConnectOrchestrator {
  return {
    connect: (req) => executeConnect(deps, req),
    disconnect: (cid) => deps.connections.disconnect(cid),
  };
}
...
function resolveIdentity(
  ...
  if (info === undefined) {
    return { ok: false, error: { code: 'dynasty_not_found', message: 'Dynasty ' + dynastyId + ' not found' } };
  }

  if (info.status !== 'active') {
    return { ok: false, error: { code: 'dynasty_inactive', message: 'Dynasty ' + dynastyId + ' is ' + info.status } };
  }
...
function spawnAndRegister(
  ...
  if (!connected) {
    return { ok: false, error: { code: 'already_connected', message: 'Connection already exists' } };
  }
...
function attemptSpawn(
  ...
  if (spawnPointId === undefined) {
    ...
    return { ok: false, error: { code: 'no_spawn_point', message: 'No spawn point in ' + identity.homeWorldId } };
  }
