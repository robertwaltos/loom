...
function spawnNewEntities(state: BridgeState, buffer: VisualStateBuffer): void {
  for (const mapped of buffer.states) {
    ...
    state.totalSpawns += 1;
    void state.deps.renderingFabric.spawnVisual(mapped.entityId, toVisualState(mapped));
  }
}

function despawnRemovedEntities(state: BridgeState, currentIds: ReadonlySet<string>): void {
...
function pushStateBatch(state: BridgeState, buffer: VisualStateBuffer): void {
