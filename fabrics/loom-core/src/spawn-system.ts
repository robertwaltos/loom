...
function validateSpawnPoint(
  ...
  const sp = state.deps.componentStore.tryGet(
    spId, 'spawn-point',
  ) as SpawnPointComponent | undefined;
...
function getSpawnTransform(state: SpawnState, spId: EntityId): TransformComponent {
  const t = state.deps.componentStore.tryGet(
    spId, 'transform',
...
function getSpawnWorldId(state: SpawnState, spId: EntityId): string {
  const wm = state.deps.componentStore.tryGet(
    spId, 'world-membership',
  ) as WorldMembershipComponent | undefined;
...
function defaultInput(): PlayerInputComponent {
  return {
    ...
    actions: [], sequenceNumber: 0,
  };
}

function defaultCamera(): CameraTargetComponent {
  return {
    mode: 'third-person', fieldOfView: 90, distance: 5,
    offset: { x: 0, y: 1.8, z: 0 }, pitchLimits: { min: -89, max: 89 },
  };
}

function defaultAnimation(): AnimationComponent {
  return { currentClip: 'Idle_Breathe', normalizedTime: 0, blendWeight: 1, playbackRate: 1, nextClip: null };
}
...
function buildPlayerComponents(
  ...
  return {
    'transform': transform,
    'identity': { displayName: params.displayName, playerId: params.playerId, faction: null, reputation: 0 },
    'movement': defaultMovement(),
    'player-input': defaultInput(),
    'camera-target': defaultCamera(),
    'visual-mesh': { meshContentHash: params.meshContentHash, assetName: params.assetName, lodTier: 'high', materialVariant: null },
    'animation': defaultAnimation(),
    'network-replication': { priority: 'critical', relevancyRadius: 200, updateFrequency: 30, ownerConnectionId: params.playerId },
...
function releaseSpawnPoint(state: SpawnState, spId: EntityId): boolean {
  ...
  const updated: SpawnPointComponent = {
    spawnType: sp.spawnType, capacity: sp.capacity,
    activeSpawns: sp.activeSpawns - 1,
    cooldownMicroseconds: sp.cooldownMicroseconds,
  };
  state.deps.componentStore.set(spId, 'spawn-point', updated);
