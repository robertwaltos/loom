...
export function createEventJournal(deps: EventJournalDeps): EventJournal {
...
function recordImpl(state: JournalState, journalName: string, event: RecordableEvent): boolean {
...
function getMetaImpl(state: JournalState, name: string): JournalMeta | undefined {
  const journal = state.journals.get(name);
  if (journal === undefined) return undefined;
