...
describe('EcsPopulationAdapter', () => {
  ...
  it('excludes NPCs in other worlds', () => {
    ...
    store.set(npcId, 'npc-tier', { tier: 1, memoryWindowDays: 90, aiBackend: 'behavior-tree', canCreateAssets: false });
    store.set(npcId, 'world-membership', {
      worldId: 'mars',
      enteredAt: 1000,
      isTransitioning: false,
  ...
  it('excludes entities without world-membership', () => {
    ...
    store.set(npcId, 'npc-tier', { tier: 0, memoryWindowDays: null, aiBackend: 'rule-based', canCreateAssets: false });
...
describe('EcsPopulationAdapter — edge cases', () => {
  ...
  it('returns multiple NPCs in same world', () => {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      store.set(id, 'npc-tier', { tier: 1, memoryWindowDays: 90, aiBackend: 'behavior-tree', canCreateAssets: false });
