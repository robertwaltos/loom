...
describe('ResourcePool — pre-warming', () => {
  ...
  it('respects max size during pre-warm', () => {
    const pool = createResourcePool(makeConfig({ initialSize: 10, maxSize: 3 }), makeObj, resetObj);
...
describe('ResourcePool — stats', () => {
  ...
  it('reports pool name', () => {
    const pool = createResourcePool(makeConfig({ name: 'entity-pool' }), makeObj, resetObj);
