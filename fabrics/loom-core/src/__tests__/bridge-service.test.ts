...
import type { BridgeServiceDeps, BridgeRenderingFabric } from '../bridge-service.js';
import type {
...
function createMockFabric(): BridgeRenderingFabric & {
  ...
  return {
    ...
    pushStateSnapshot: (updates) => {
      pushes.push({ count: updates.length });
    },
    spawnVisual: (id) => {
      spawned.push(id);
...
function createDeps(buffer: VisualStateBuffer | undefined): {
