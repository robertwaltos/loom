...
describe('EventCorrelationEngine — add events', () => {
  ...
  it('adds an event with explicit correlation ID', () => {
    ...
    const event = engine.addEvent({
      eventId: 'e1', eventType: 'spawn', correlationId: 'session-abc',
    });
...
describe('EventCorrelationEngine — correlation groups', () => {
  it('creates a group for new correlation ID', () => {
    const engine = createEventCorrelationEngine(makeDeps());
    engine.addEvent({
      eventId: 'e1', eventType: 'a', correlationId: 'g1',
    });
  ...
  it('adds events to existing group', () => {
    ...
    engine.addEvent({
      eventId: 'e2', eventType: 'b', correlationId: 'g1', causationId: 'e1',
    });
    const group = engine.getGroup('g1');
...
describe('EventCorrelationEngine — causation chains', () => {
  it('builds a chain from root to leaf', () => {
    ...
    engine.addEvent({
      eventId: 'e2', eventType: 'child', correlationId: 'g1', causationId: 'e1',
    });
    engine.addEvent({
      eventId: 'e3', eventType: 'grandchild', correlationId: 'g1', causationId: 'e2',
    });
  ...
  it('gets children of an event', () => {
    ...
    engine.addEvent({ eventId: 'e1', eventType: 'root', correlationId: 'g1' });
    engine.addEvent({
      eventId: 'e2', eventType: 'c1', correlationId: 'g1', causationId: 'e1',
    });
    engine.addEvent({
      eventId: 'e3', eventType: 'c2', correlationId: 'g1', causationId: 'e1',
    });
  ...
  it('computes depth correctly', () => {
    ...
    engine.addEvent({
      eventId: 'e3', eventType: 'c', correlationId: 'g1', causationId: 'e2',
    });
