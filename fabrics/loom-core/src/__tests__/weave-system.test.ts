...
function mockCompletions(entries?: WeaveCompletedTransit[]): WeaveTransitCompletionPort {
  const queue = entries ?? [];
  return {
...
describe('WeaveSystem — transit completion', () => {
  it('updates world-membership on completed transit', () => {
    ...
    const completions = mockCompletions([{ entityId: 'ship-1', destinationNodeId: 'mars-node' }]);
...
describe('WeaveSystem — multi-transit and drain', () => {
  ...
  it('drains completions so they are not reprocessed', () => {
    ...
    const completions = mockCompletions([{ entityId: 'ship-1', destinationNodeId: 'mars' }]);
...
describe('WeaveSystem — event emission', () => {
  it('emits transition.completed event on transit', () => {
    ...
    const system = createWeaveSystem({
      orchestrator: mockOrchestrator(),
      completions: mockCompletions([{ entityId: 'ship-1', destinationNodeId: 'mars' }]),
      componentStore: store,
      clock: mockClock(),
      events: {
        publish: (e) => {
