...
function makeEvent(id: string, type: string, ts: number): ReplayEvent {
  return {
    eventId: id,
    eventType: type,
    payload: '{}',
...
describe('EventReplayService — replay execution', () => {
  it('replays events through registered handlers', () => {
    const events = [makeEvent('e1', 'entity.created', 100), makeEvent('e2', 'entity.created', 200)];
  ...
  it('skips events without handlers', () => {
    const events = [makeEvent('e1', 'entity.created', 100), makeEvent('e2', 'world.loaded', 200)];
    const service = createEventReplayService(makeDeps(events));
    service.registerHandler('entity.created', () => {});
    const session = service.replay({});
