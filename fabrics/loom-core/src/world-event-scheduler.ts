...
export function createWorldEventScheduler(deps: WorldEventSchedulerDeps): WorldEventScheduler {
...
function fireEvent(state: SchedulerState, event: MutableEvent, names: string[]): void {
  event.firedCount += 1;
  state.totalFired += 1;
...
function listPendingImpl(state: SchedulerState): ReadonlyArray<ScheduledEvent> {
...
function toReadonly(event: MutableEvent | undefined): ScheduledEvent | undefined {
  return event;
}
