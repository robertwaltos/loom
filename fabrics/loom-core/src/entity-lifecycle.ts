...
export function createEntityLifecycleManager(
  ...
  return {
    ...
    onTransition: (cb) => { state.callbacks.push(cb); },
...
function validateTransition(
  ...
  if (!allowed.includes(to)) {
    throw new Error(
      'Invalid lifecycle transition for ' + entityId
      + ': ' + from + ' -> ' + to,
    );
  }
}
...
function computeStats(state: ManagerState): LifecycleStats {
  ...
  for (const record of state.records.values()) {
    switch (record.phase) {
      ...
      case 'migrating': migratingCount++; break;
