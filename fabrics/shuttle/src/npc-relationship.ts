...
export interface RelationshipTracker {
  ...
  adjustDisposition(fromId: string, toId: string, delta: number): DispositionChange;
...
export function createRelationshipTracker(deps: RelationshipTrackerDeps): RelationshipTracker {
...
function formImpl(state: TrackerState, params: FormRelationshipParams): Relationship {
  ...
  const disposition = clampDisposition(state, params.initialDisposition ?? 0);
...
function interactionImpl(state: TrackerState, fromId: string, toId: string): boolean {
  const key = relationshipKey(fromId, toId);
  const rel = state.relationships.get(key);
...
function getImpl(state: TrackerState, fromId: string, toId: string): Relationship | undefined {
  const key = relationshipKey(fromId, toId);
  const rel = state.relationships.get(key);
  return rel !== undefined ? toReadonly(rel) : undefined;
}

function fromImpl(state: TrackerState, entityId: string): ReadonlyArray<Relationship> {
...
function toImpl(state: TrackerState, entityId: string): ReadonlyArray<Relationship> {
  const results: Relationship[] = [];
  for (const rel of state.relationships.values()) {
...
function mutualImpl(state: TrackerState, entityId: string): ReadonlyArray<Relationship> {
  const results: Relationship[] = [];
  for (const rel of state.relationships.values()) {
    const involved = rel.fromEntityId === entityId || rel.toEntityId === entityId;
    if (involved) {
      results.push(toReadonly(rel));
    }
  }
...
function removeImpl(state: TrackerState, fromId: string, toId: string): boolean {
