...
function adjustAffinityImpl(
  state: FactionState,
  params: AdjustAffinityParams,
): FactionAffinity {
...
function getAllAffinitiesImpl(
  state: FactionState,
  entityId: string,
): readonly FactionAffinity[] {
...
function getDominantFactionImpl(
  state: FactionState,
  entityId: string,
): string | undefined {
  const map = state.affinities.get(entityId);
...
function createNpcFactionTracker(): NpcFactionTracker {
  ...
  return {
    ...
    getAffinity: (eid, fid) =>
      state.affinities.get(eid)?.get(fid) ?? 0,
    getAffinityLevel: (eid, fid) =>
      classifyAffinity(state.affinities.get(eid)?.get(fid) ?? 0),
