...
export function createNpcEmotionModel(deps: EmotionModelDeps): NpcEmotionModel {
...
function stimulusImpl(state: ModelState, params: StimulusParams): StimulusResult {
...
function snapshotImpl(state: ModelState, entityId: string): EmotionSnapshot | undefined {
...
function dominantImpl(state: ModelState, entityId: string): EmotionEntry | undefined {
  const entity = state.entities.get(entityId);
  if (entity === undefined) return undefined;
...
function findDominant(entries: ReadonlyArray<EmotionEntry>): EmotionEntry | undefined {
