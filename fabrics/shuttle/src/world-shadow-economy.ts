...
const COMMODITY_TYPES: ReadonlyArray<CommodityType> = ['food', 'materials', 'services', 'luxury'];
...
export function createWorldShadowEconomy(
  ...
  return {
    ...
    recordProduction: (c, a) => {
      addSupply(state, c, a);
    },
    recordConsumption: (c, a) => {
      addDemand(state, c, a);
    },
    setLaborMarket: (d, s) => {
      setLabor(state, d, s);
...
function snapshotCommodities(state: EconomyState): Readonly<Record<CommodityType, CommodityState>> {
