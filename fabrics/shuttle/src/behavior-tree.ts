...
export function createSequenceNode(name: string, children: ReadonlyArray<BtNode>): BtNode {
  let runningIndex = 0;
...
export function createSelectorNode(name: string, children: ReadonlyArray<BtNode>): BtNode {
  let runningIndex = 0;
...
export function createActionNode(name: string, action: BtActionFn): BtNode {
  return {
    name,
    type: 'action',
    tick: (ctx) => action(ctx),
    reset: () => {
      /* action nodes are stateless */
    },
  };
}

export function createConditionNode(name: string, condition: BtConditionFn): BtNode {
  return {
    name,
    type: 'condition',
    tick: (ctx) => (condition(ctx) ? 'success' : 'failure'),
    reset: () => {
      /* condition nodes are stateless */
    },
  };
}
...
export function createBehaviorTree(name: string, root: BtNode): BehaviorTree {
  return {
    name,
...
function registerTree(trees: Map<string, TreeEntry>, tree: BehaviorTree): void {
  trees.set(tree.name, {
    tree,
    totalTicks: 0,
    successCount: 0,
...
function getTreeStats(trees: Map<string, TreeEntry>, name: string): BtTreeStats | undefined {
  const entry = trees.get(name);
