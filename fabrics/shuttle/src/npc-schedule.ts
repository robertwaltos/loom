...
export function createNpcScheduleManager(
  deps: NpcScheduleDeps,
): NpcScheduleManager {
...
function getScheduleImpl(
  state: ScheduleState,
  entityId: string,
): NpcSchedule | undefined {
  const schedule = state.schedules.get(entityId);
