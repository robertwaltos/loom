...
function shuttleTick(
  ...
  updateCumulativeStats(
    state,
    npcs.length,
    tier1.acted + tier2.acted + tier3.acted,
    memoriesRecorded,
  );
...
function treeResultToDecision(npcId: string, result: ShuttleTreeResult): ShuttleDecision {
  if (result === 'success')
    return { npcId, actionType: 'tree-complete', outcome: 'act', confidence: 1.0 };
  if (result === 'running')
    return { npcId, actionType: 'tree-running', outcome: 'defer', confidence: 0.5 };
...
function buildRequest(state: OrchestratorState, npc: ShuttleNpcRecord): ShuttleDecisionRequest {
...
function shouldRecordMemory(state: OrchestratorState, decision: ShuttleDecision): boolean {
