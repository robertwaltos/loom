...
interface NpcDialogueManager {
  ...
  readonly startConversation: (params: StartConversationParams) => Conversation | undefined;
  readonly selectResponse: (
    conversationId: string,
    responseId: string,
  ) => SelectResponseResult | undefined;
  readonly abandonConversation: (conversationId: string) => boolean;
  readonly getConversation: (conversationId: string) => Conversation | undefined;
  readonly getActiveByPlayer: (playerEntityId: string) => Conversation | undefined;
...
function registerTreeImpl(state: DialogueState, tree: DialogueTree): boolean {
...
function selectResponseImpl(
  ...
  const response = currentNode.responses.find((r) => r.responseId === responseId);
...
function abandonConversationImpl(state: DialogueState, conversationId: string): boolean {
  const conv = state.conversations.get(conversationId);
...
function getConversationImpl(state: DialogueState, id: string): Conversation | undefined {
  const c = state.conversations.get(id);
  return c ? toConversation(c) : undefined;
}

function getActiveByPlayerImpl(state: DialogueState, pid: string): Conversation | undefined {
...
function createNpcDialogueManager(deps: NpcDialogueDeps): NpcDialogueManager {
