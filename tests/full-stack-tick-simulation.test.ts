...
function buildShuttleDeps(populationPort: {
  readonly listActiveNpcs: (
    w: string,
  ) => ReadonlyArray<{ npcId: string; worldId: string; tier: number; displayName: string }>;
...
function copyNpcsToStore(store: { set(eid: EntityId, ct: string, data: unknown): void }): void {
  for (let i = 1; i <= 5; i++) {
...
function runTicks(orchestrator: ReturnType<typeof createGameOrchestrator>, count: number): void {
