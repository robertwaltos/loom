...
function buildWeaveDeps(): {
  ...
  const entries: WeaveQueueEntry[] = [
    {
      requestId: 'rq-1',
      entityId: 'ship-1',
      originNodeId: 'earth-node',
      destinationNodeId: 'mars-node',
      priority: 'normal',
    },
  ];
  ...
  const deps: WeaveOrchestratorDeps = {
    ...
    coherence: {
      computeCoherence: (cId) => {
        const f = corState.coherenceMap.get(cId);
        return f ?? 0.5;
      },
    },
    ...
    ledger: {
      recordTransit: (p) => {
        ledgerRecords.push({ entityId: p.entityId, status: p.status });
        return 'rec-' + String(ledgerRecords.length);
      },
    },
    clock: createSharedClock(),
  };
