...
export function createSchemaRegistry(): SchemaRegistry {
  ...
  return {
    ...
    currentVersion: () => {
      if (store.latest === undefined) throw new Error('No versions registered');
      return store.latest;
    },
    listDescriptors: () =>
      Object.freeze([...store.descriptors.values()].sort((a, b) => (a.version as number) - (b.version as number))),
  };
}
